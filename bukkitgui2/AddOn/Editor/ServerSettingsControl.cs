...
namespace Net.Bertware.Bukkitgui2.AddOn.Editor
{
	public partial class ServerSettingsControl : MetroUserControl
	{
		...
		{
			Dictionary<string, string> settings = ServerConfig.ServerSettings;
		...
		private void btnEdit_Click(object sender, EventArgs e)
		{
			...
			string value = ServerConfig.GetServerSetting(key);

			SettingsEditDialog dialog = new SettingsEditDialog(key,value);
			if (dialog.ShowDialog() != DialogResult.OK) return;
			ServerConfig.SetServerSetting(dialog.Setting,dialog.Value);
			UpdateListView();
		}
