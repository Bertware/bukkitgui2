	...
	public partial class PluginUpdater : MetroForm
	{
		...
		private void LoadAsync()
		{
			try
			{
				...
				for (UInt16 i = 0; i <= InstalledPlugins.Count - 1; i++)
				{
					...
					{
						SetStatus(T.Tr("Loading plugin data:") + plugin.Name + "(" + i + 1 + "/" + InstalledPlugins.Count + ")");
				...
				else
				{
					...
					foreach (KeyValuePair<InstalledPlugin, BukgetPlugin> entry in _pluginLinkDictionary)
					{
						...
						catch (Exception ex)
						{
							...
							{
								Logger.Log(LogLevel.Warning, "PluginUpdater", "Couldn't load plugin:" + entry.Key.Name, ex.Message);
							}
							else
							{
			...
			try
			{
				...
				else
				{
					...
					foreach (ListViewItem item in SlvPlugins.CheckedItems)
					{
						if (item.Tag != null && Equals(item.Tag.ToString(), "TRUE"))
						{
							foreach (KeyValuePair<InstalledPlugin, BukgetPlugin> pair in _pluginLinkDictionary)
							{
								try
								{
									...
									{
										if (pair.Value == null || pair.Value.VersionsList == null || pair.Value.VersionsList[0] == null)
											continue;
										if (this != null && !(Disposing | IsDisposed) && !(lblStatus.IsDisposed | lblStatus.Disposing))
											SetStatus(T.Tr("Updating plugin") + " " + pair.Key.Name + " " + T.Tr("to version") + " " +
								...
								{
									Logger.Log(LogLevel.Warning, "PluginUpdater", "Exception in Update routine for " + pair.Key.Name, ex.Message);
								}
							}
						}
						else
						{
							if (this != null && !(Disposing | IsDisposed) && !(lblStatus.IsDisposed | lblStatus.Disposing))
								SetStatus(T.Tr("Skipping plugin") + " " + item.SubItems[0].Text + " - " + T.Tr("cannot update") + "(" + i +
		...
		public int CheckVersion(string oldversion, string newversion)
		{
			try
			{
				...
				if (oldversion == null || string.IsNullOrEmpty(oldversion) || newversion == null || string.IsNullOrEmpty(newversion))
				{
