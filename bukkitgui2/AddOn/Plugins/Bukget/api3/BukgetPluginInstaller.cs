...
// Created 2014/07/09
// Last edited at 2014/07/09 18:48
...
namespace Net.Bertware.Bukkitgui2.AddOn.Plugins.Bukget.api3
{
    internal class BukgetPluginInstaller
    {
            ...
            bool ShowUI = true)
        {
            ...
            {
                return InstallZip(version, targetlocation, updatelist, ShowUI);
            }
            MessageBox.Show(
        ...
        /// <param name="ShowUI">Allow pop-up dialogs</param>
        /// <remarks></remarks>
        private static Boolean InstallZip(BukgetPluginVersion version, string targetlocation = "",
            bool updatelist = true,
            bool ShowUI = true)
        {
            if (ShowUI)
            ...
            foreach (DirectoryInfo directoryInZipInfo in extracteDirectoryInfo.GetDirectories())
            {
                ...
                foreach (DirectoryInfo Dir_2 in directoryInZipInfo.GetDirectories())
                {
                    bool copy_2 = false;
                    foreach (string f in extractedFileNamesList)
                    {
                        if (!f.Contains(Dir_2.Name)) continue;
                        copy_2 = true;
                        Logger.Log(LogLevel.Info, "BukgetAPI",
                            "Config/Info folder found in .zip, marked directory for copy:" + Dir_2.Name);
                    }
                    foreach (FileInfo fileInfo in Dir_2.GetFiles())
                    {
                        if (fileInfo.Extension == ".txt" | fileInfo.Extension == ".yml" | fileInfo.Extension == ".cfg" |
                            fileInfo.Extension == ".csv" | fileInfo.Extension == ".js")
                        {
